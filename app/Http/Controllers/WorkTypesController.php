...
class WorkTypesController extends Controller
{
    ...
    public function worktype(Request $request)
    {
        ...
        if ($search_query != '') {
            ...
            $api_response = makeIntervalsApiCall(Auth::user()->intervals_api_key, 'GET', 'worktype/?' . $search_query);
        } else {

            $api_response = makeIntervalsApiCall(Auth::user()->intervals_api_key, 'GET', 'worktype/');
        }
